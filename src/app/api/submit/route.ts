...
function formatTelegramMessage(a: OnboardingAnswers) {
  const lines = [
    ...
    `Daily practice: ${a.dailyStudy ?? "-"}`,
    `Preferred time: ${a.studyTimeOfDay ?? "-"}`,
...
export async function POST(req: Request) {
  try {
    ...
    const answers: OnboardingAnswers = {
      ...
      dailyStudy: (b.dailyStudy ?? null) as OnboardingAnswers["dailyStudy"],
      studyTimeOfDay: (b.studyTimeOfDay ?? null) as OnboardingAnswers["studyTimeOfDay"],
      priorExperience: (b.priorExperience ?? null) as OnboardingAnswers["priorExperience"],
