...
/** Size/weight/leading for option labels (aligned across list + multi + language tiles). */
export const funnelOptionTypeClass = "text-base font-normal leading-snug sm:text-lg sm:leading-snug";

/** Full option label line (ink). */
export const funnelOptionLabelClass = `${funnelOptionTypeClass} text-funnel-ink`;

/** Shared row chrome for list-style choice buttons. */
const funnelOptionRowBaseClass =
  "flex min-h-[60px] w-full min-w-0 touch-manipulation items-center justify-between gap-3 rounded-xl border bg-funnel-surface px-4 py-3.5 text-left font-inherit transition active:scale-[0.99] sm:min-h-[62px] sm:gap-4";
          ...
          >
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className={funnelOptionLabelClass}>{opt.label}</div>
                {opt.hint ? <div className="mt-0.5 text-base font-normal text-funnel-muted">{opt.hint}</div> : null}
              </div>
            </div>
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(opt.value)}
            className={[
              "relative flex min-h-[6rem] touch-manipulation flex-col items-center justify-center gap-1 rounded-xl border bg-funnel-surface px-2 py-3 pt-7 text-center font-inherit transition active:scale-[0.99] sm:min-h-[6.5rem]",
              selected ? "border-funnel-primary bg-funnel-selected" : "border-funnel-border hover:border-funnel-muted/50",
            ].join(" ")}
          >
            ...
            <span className={["line-clamp-2 text-center", funnelOptionLabelClass].join(" ")}>
              {opt.label}
            </span>
          </button>
        );
